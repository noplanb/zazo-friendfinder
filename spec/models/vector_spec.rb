...
RSpec.describe Vector, type: :model do
  describe 'validations' do
    ...
    describe '#additions_must_be_allowed' do
      ...
      context 'valid' do
        context 'mobile' do
          let(:vector) { FactoryGirl.build :vector_mobile, additions: { sms_messages_sent: 15 } }

          it { expect(vector).to be_valid }
        end

        context 'email' do
          let(:vector) { FactoryGirl.build :vector_email, additions: { email_messages_sent: 65 } }

          it { expect(vector).to be_valid }
        end

        context 'facebook' do
          let(:vector) { FactoryGirl.build :vector_facebook }
      ...
      context 'invalid' do
        context 'mobile' do
          let(:vector) { FactoryGirl.build :vector_mobile, additions: { email_messages_sent: 15 } }
        ...
        context 'email' do
          let(:vector) { FactoryGirl.build :vector_email, additions: { sms_messages_sent: 65 } }
        ...
        context 'facebook' do
          let(:vector) { FactoryGirl.build :vector_facebook, additions: { marked_as_favorite: true } }
    ...
    describe '#value_has_correct_format' do
      ...
      context 'valid mobile' do
        let(:vector) { FactoryGirl.build :vector_mobile }

        it { expect(vector).to be_valid }
      end

      context 'invalid mobile' do
        let(:vector) { FactoryGirl.build :vector_mobile, value: 'xxxxxxxxx' }
      ...
      context 'valid email' do
        let(:vector) { FactoryGirl.build :vector_email }

        it { expect(vector).to be_valid }
      end

      context 'invalid email' do
        let(:vector) { FactoryGirl.build :vector_email, value: 'asdasd@asdasd' }
