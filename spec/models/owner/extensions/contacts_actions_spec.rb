...
RSpec.describe Owner::Extensions::ContactsActions, type: :model do
  ...
  describe '#find_contact_and_update_info',
    vcr: { cassette: 'find_contact_and_update_info' } do
    let(:mobile) { '+16502453537' }
    let(:email) { 'admin@google.com' }

    let!(:contact_1) do
      create(:contact, owner_mkey: owner_mkey,
        vectors: [create(:vector_mobile, value: mobile)])
    end
    let!(:contact_2) do
      create(:contact, owner_mkey: owner_mkey,
        vectors: [create(:vector_email, value: email)])
    end
  ...
  describe '#recalculate_scores',
    vcr: { cassette: 'recalculate_scores' } do
    before do
      # first owner's contact
      vectors = [
        create(:vector_mobile, additions: { sms_messages_sent: 12 }),
        create(:vector_email)]
      create :contact, owner_mkey: owner_mkey, vectors: vectors, additions: { marked_as_favorite: true }

      # second owner's contact
      vectors = [create(:vector_mobile)]
      create :contact, owner_mkey: owner_mkey, vectors: vectors, additions: { marked_as_favorite: true }

      # non owner's contact
      vectors = [create(:vector_mobile, value: vectors.last.value)]
      create :contact, vectors: vectors
    end
